...
class GRU(nn.Module):
    r"""
    GRU 模块, 轻量封装的Pytorch GRU. 在提供seq_len的情况下，将自动使用pack_padded_sequence; 同时默认将forget gate的bias初始化
    为1; 且可以应对DataParallel中LSTM的使用问题。
    ...
    def __init__(self, input_size, hidden_size=100, num_layers=1, dropout=0.0, batch_first=True,
        ...
        self.batch_first = batch_first
        self.lstm = nn.GRU(input_size, hidden_size, num_layers, bias=bias, batch_first=batch_first,
                            dropout=dropout, bidirectional=bidirectional)
        self.init_param()
